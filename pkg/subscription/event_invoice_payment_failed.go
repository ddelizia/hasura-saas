...
func ProcessInvoicePaymentFailed(c context.Context, event stripe.Event, id string, sdkSvc gqlsdk.Service) error {
	...
	logrus.Debug("get account id for subscription")
	accountInfo, err := sdkSvc.GetAccountFromSubscription(c, data.Subscription.ID)
	if err != nil {
		logrus.WithError(err).WithField("subscription", data.Subscription.ID).Error("unable to get account id")
		return errorx.InternalError.Wrap(err, "unable to get account id")
	}
	if len(accountInfo.SubscriptionStatus) != 1 {
		logrus.WithField("subscription", data.Subscription.ID).Error("multiple accounts have been returned")
		return errorx.InternalError.New("unable to get account id")
	}

	logrus.Debug("get subscription information from invoice")
	subscriptionParams := &stripe.SubscriptionParams{}
	ser, err := sub.Get(data.Subscription.ID, subscriptionParams)
	if err != nil {
		logrus.WithError(err).WithField("subscription", data.Subscription.ID).Error("unable to get subscription")
		return errorx.InternalError.Wrap(err, "unable to get subscription")
	}

	logrus.Debug("update status od the subscription on hasura")
