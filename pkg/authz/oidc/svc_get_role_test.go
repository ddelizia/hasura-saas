...
var _ = Describe("RoleGetterOidc", func() {

	logrus.SetOutput(ioutil.Discard)

	var (
		s *oidc.RoleGetterOidc
	)
	...
	Context("GetRole()", func() {
		...
		It("should return the role when validation is successful", func() {
			// Given
			...
			oidc.ValidateJwtFunc = func(jwtB64 string, r *http.Request) (*oidc.JwtTokenWithClaims, error) {
				return &oidc.JwtTokenWithClaims{HasuraSaasNamespace: oidc.HasuraSaasClaims{HasuraSaasRole: "role"}}, nil
			}

			// When
			got, err := s.GetRole(httpReq)
		...
		It("should return the anonymous role when there is no token", func() {
			// When
			httpReq := &http.Request{
				Header: http.Header{},
			}
